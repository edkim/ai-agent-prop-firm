/**
 * Backfill 3 months of daily bars for all tickers in ohlcv_data
 *
 * Usage: npx ts-node helper-scripts/backfill-daily-3months.ts
 */
...
/**
 * Get date range for backfill (3 months = 90 days)
 */
...
/**
 * Get tickers from ohlcv_data table
 */
function getTickersFromDatabase(): string[] {
  const db = getDatabase();
 ...
 */
async function main() {
  console.log('🚀 Starting Daily Data Backfill (3 Months)\n');
  ...
  const tickers = getTickersFromDatabase();
  console.log(`📊 Found ${tickers.length} tickers in ohlcv_data table\n`);

  if (tickers.length === 0) {
    console.error('❌ No tickers found in ohlcv_data table');
    process.exit(1);
  }

  // Calculate date range (3 months = 90 days)
  const { startDate, endDate } = getDateRange(90);
  console.log(`📅 Date range: ${startDate} to ${endDate} (3 months)\n`);
