/**
 * Backfill 30 days of 1-minute intraday data for S&P Technology Sector
 *
 * This will fetch approximately 800,000 bars for the tech sector.
 * Used to validate that learned patterns work at higher frequency.
 *
 * Usage: npx ts-node helper-scripts/backfill-tech-sector-1min.ts
 */
...
const WATCHLIST_TICKERS = process.env.WATCHLIST_TICKERS || '';

// Parallel processing - unlimited API calls
const CONCURRENT_REQUESTS = 10; // Process 10 tickers at once
 ...
 */
async function main() {
  console.log('🚀 Starting S&P Tech Sector 1-Minute Intraday Data Backfill\n');
  console.log('⚠️  WARNING: This will fetch approximately 800,000 bars for 30 days of data\n');

  // Initialize database - use project root database
  const dbPath = process.env.DATABASE_PATH || path.resolve(__dirname, '../../backtesting.db');
  initializeDatabase(dbPath);

  // Parse ticker list
  if (!WATCHLIST_TICKERS) {
    console.error('❌ WATCHLIST_TICKERS not set in .env');
    process.exit(1);
  }

  const tickers = WATCHLIST_TICKERS.split(',').map(t => t.trim()).filter(t => t.length > 0);
  console.log(`📊 Found ${tickers.length} tickers in WATCHLIST_TICKERS\n`);

  // Calculate date range (30 days back)
  const { startDate, endDate } = getDateRange(30);
  console.log(`📅 Date range: ${startDate} to ${endDate}`);
  console.log(`⏱️  Expected bars per ticker: ~2,340 (30 days × 78 bars/day)\n`);
  ...
  const processBatch = async (batch: string[], batchNum: number) => {
    ...
    const results = await Promise.all(
      batch.map(async (ticker) => {
        try {
          const bars = await fetch1MinBars(ticker, startDate, endDate);
          if (bars.length > 0) {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    console.log(`\n⏱️  Progress: ${progress}% | Elapsed: ${elapsedMinutes} min | Total bars: ${totalBars.toLocaleString()}`);
  }
