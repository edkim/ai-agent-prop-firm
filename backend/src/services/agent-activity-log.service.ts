 ...
 * Manages the learning_agent_activity_log table for audit trail and debugging
 */
...
export class AgentActivityLogService {
  ...
  async log(entry: ActivityLogEntry): Promise<void> {
    ...
    db.prepare(`
      INSERT INTO learning_agent_activity_log (id, learning_agent_id, activity_type, ticker, description, data, timestamp)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      uuidv4(),
      entry.learning_agent_id,
      entry.activity_type,
      entry.ticker || null,
      entry.description,
      entry.data || null,
  ...
  getAgentLogs(agentId: string, limit: number = 100): ActivityLog[] {
    ...
    return db.prepare(`
      SELECT *
      FROM learning_agent_activity_log
  ...
  deleteOldLogs(daysToKeep: number = 30): number {
    ...
    const result = db.prepare(`
      DELETE FROM learning_agent_activity_log
      WHERE timestamp < ?
