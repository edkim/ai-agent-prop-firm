...
export default function BacktestSets({
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 h-full overflow-y-auto flex flex-col">
      ...
      {activeBacktestSet && activeSamples.length > 0 && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded">
          ...
          <div className="text-xs text-gray-600 mb-2">
            Select 1-3 samples to analyze with Claude AI
          </div>
          <button
            onClick={handleOpenAnalysisModal}
            disabled={selectedSampleIds.size === 0 || selectedSampleIds.size > 3}
          ...
          >
            Analyze {selectedSampleIds.size > 0 ? `${selectedSampleIds.size} ` : ''}Sample{selectedSampleIds.size !== 1 ? 's' : ''}
          </button>
        </div>
      )}
            ...
            {activeSamples.map((sample) => (
              ...
              >
                <input
                  ...
                  disabled={!selectedSampleIds.has(sample.id) && selectedSampleIds.size >= 3}
      ...
      {/* Analysis Modal */}
      {showAnalysisModal && activeBacktestSet && (
        <AnalysisModal
